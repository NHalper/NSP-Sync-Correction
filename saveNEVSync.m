function saveNEVSync(NEV, varargin)
...
%               DEFAULT: -modified.nev will be added to the end of the current
%               file name.
%
...
%
% 1.2.0.0: Nick Halper
%   - Fixed a bug where choosing to not overwrite would not actually create
%   a new filename, but would crash the script..
%
% 1.3.0.0: Nick halper
%   - Fixed an issue where the blank label in non-sequential electrode info
%   structures would cause issues with the case statement and header offset
%   values.
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if ~exist('FilePath',   'var'); FilePath = [fullfile(NEV.MetaTags.FilePath,NEV.MetaTags.Filename) '-Aligned-0.nev']; end
...
HeaderOffsetTracker = ftell(FileID);
...
BytesInExtendedHeaderTracker = ftell(FileID);
fwrite(FileID,ExtendedHeaderBytes/32,'uint32');

EndOfBasicHeaderTesting = ftell(FileID);
...
if isfield(NEV,'ElectrodesInfo')
    ...
    %in  NEUEVWAV packets. 
        for IDX = 1:length(NEV.ElectrodesInfo)
            if ~isempty(NEV.ElectrodesInfo(IDX).ConnectorBank)
                Before = ftell(FileID);
...
EndOfExtendedHeaderTesting = ftell(FileID);

NumberOfBytesInExtendedHeaderTesting = EndOfExtendedHeaderTesting-EndOfBasicHeaderTesting;

EOExtendedHeaderTracker = ftell(FileID);

fseek(FileID,HeaderOffsetTracker,'bof');
fwrite(FileID,EOExtendedHeaderTracker,'uint32');

fseek(FileID,BytesInExtendedHeaderTracker,'bof');
fwrite(FileID,NumberOfBytesInExtendedHeaderTesting/32,'uint32');

fseek(FileID,EOExtendedHeaderTracker,'bof');
%%
% Write Data 
